...
-- Updated function to only count specific leave types (Annual Leave and Working Holiday)
-- when updating leave allocation on approval
...
BEGIN
  ...
  should_deduct := (NEW.type = 'Annual Leave' OR NEW.type = 'Working Holiday');
  
  -- Only proceed if status changed to 'Approved' AND this is a leave type that deducts days
  IF NEW.status = 'Approved' AND (OLD.status IS NULL OR OLD.status <> 'Approved') AND should_deduct THEN
    ...
    IF leave_period_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        -- Calculate total used days from all approved leave requests WITH SPECIFIC TYPES
        SELECT COALESCE(SUM(duration), 0) INTO total_used_days
        FROM public.leave_requests
        WHERE 
          employee_id = NEW.employee_id AND
          (type = 'Annual Leave' OR type = 'Working Holiday') AND
          status = 'Approved' AND
